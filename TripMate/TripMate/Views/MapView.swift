...
struct MapView: View {
    ...
    var body: some View {
        ZStack(alignment: .top) {
            ...
            if viewModel.isNavigating {
                // Navigation mode — show step banner + bottom bar only
                navigationBanner
                navigationBottomBar
                exitNavigationButton
            } else {
                // Normal mode
                searchAndSuggestionsLayer
                routeInfoLayer
                controlsLayer
                styleButtonLayer
            }
    ...
    private var mapLayer: some View {
        MapReader { proxy in
            ...
            .onTapGesture { screenPoint in
                ...
                } else if let coord = proxy.convert(screenPoint, from: .local) {
                    viewModel.reverseGeocode(coord, userLocation: locationManager.userLocation)
                }
            }
            .onReceive(locationManager.$userLocation) { location in
                guard let location = location else { return }
                viewModel.handleFirstLocation(location)
                if viewModel.isNavigating {
                    viewModel.updateNavigationStep(userLocation: location)
                }
            }
        }
    }
    ...
    private var searchAndSuggestionsLayer: some View {
        VStack(spacing: 0) {
            HStack(spacing: 12) {
                Image(systemName: "magnifyingglass").foregroundColor(.secondary)

                TextField("Search or tap map for directions...", text: $viewModel.searchText)
                    .submitLabel(.search)
                    .focused($isSearchFocused)
                    .onSubmit {
                        isSearchFocused = false
                        if let first = viewModel.searchSuggestions.first {
                            viewModel.selectSuggestion(first, userLocation: locationManager.userLocation)
                        }
                    }
            ...
            if !viewModel.searchSuggestions.isEmpty && isSearchFocused {
                VStack(spacing: 0) {
                    ForEach(viewModel.searchSuggestions, id: \.self) { suggestion in
                        Button {
                            isSearchFocused = false
                            viewModel.selectSuggestion(suggestion, userLocation: locationManager.userLocation)
                        } label: {
                            HStack(spacing: 12) {
                                Image(systemName: "mappin.circle.fill")
                                    .foregroundColor(.brown)
    ...
    private var routeInfoLayer: some View {
        if viewModel.showRouteInfo, viewModel.route != nil {
            VStack {
                ...
                VStack(spacing: 16) {
                    RoundedRectangle(cornerRadius: 3)
                        .fill(Color.gray.opacity(0.4)).frame(width: 40, height: 4)
                    ...
                    .onChange(of: viewModel.selectedTravelMode) {
                        if let loc = locationManager.userLocation {
                            viewModel.getDirections(userLocation: loc)
                        }
                    }
                    ...
                    VStack(spacing: 10) {
                        ...
                        Button { viewModel.startNavigation() } label: {
                            HStack(spacing: 8) {
                                Image(systemName: "arrow.triangle.turn.up.right.circle.fill")
                                    .font(.system(size: 16, weight: .semibold))
                            ...
                            .background(Color.brown)
                            .cornerRadius(14)
                            .shadow(color: .brown.opacity(0.4), radius: 6, x: 0, y: 3)
                        }
    ...
    // MARK: - Navigation Banner (top — current step)
        ...
        VStack(spacing: 0) {
            HStack(spacing: 14) {
                ...
                ZStack {
                    ...
                    Image(systemName: directionIcon(for: viewModel.currentStepInstruction))
                        .font(.system(size: 20, weight: .bold))
                        .foregroundColor(.white)
                }

                VStack(alignment: .leading, spacing: 3) {
                    Text(viewModel.currentStepInstruction.isEmpty ? "Follow the route" : viewModel.currentStepInstruction)
                        .font(.system(size: 15, weight: .semibold))
                        .foregroundColor(.primary)
                        .lineLimit(2)
                    if !viewModel.currentStepDistance.isEmpty {
                        Text("In \(viewModel.currentStepDistance)")
                            .font(.caption)
                ...
                VStack(spacing: 2) {
                    Text("\(viewModel.currentStepIndex + 1)")
                        ...
                        .foregroundColor(.brown)
                    Text("of \((viewModel.route?.steps.filter { !$0.instructions.isEmpty }.count ?? 1))")
                        .font(.caption2)
                        .foregroundColor(.secondary)
                }
            }
    ...
    // MARK: - Navigation Bottom Bar (remaining + prev/next)
            ...
            VStack(spacing: 14) {
                ...
                HStack(spacing: 12) {
                    Button { viewModel.previousStep() } label: {
                        ...
                        .foregroundColor(viewModel.currentStepIndex == 0 ? .secondary : .primary)
                        .frame(maxWidth: .infinity)
                        .padding(.vertical, 12)
    ...
    private var exitNavigationButton: some View {
        VStack {
            ...
            HStack {
                ...
                .padding(.leading, 16)
                .padding(.bottom, 270)     // sits above the bottom card
    ...
    private var controlsLayer: some View {
        ...
        .padding(.trailing, 16)
        .padding(.bottom, viewModel.showRouteInfo ? 320 : 100)
        .frame(maxWidth: .infinity, alignment: .trailing)
        .animation(.easeInOut(duration: 0.25), value: viewModel.showRouteInfo)
    }
    ...
    private var styleButtonLayer: some View {
        VStack {
            ...
            HStack {
                Spacer()
                Button {
                    viewModel.selectedMapStyle = (viewModel.selectedMapStyle + 1) % 3
                } label: {
                    Image(systemName: viewModel.styleIcon)
                        .font(.system(size: 20, weight: .medium))
                        .foregroundColor(.primary)
    ...
    // MARK: - Steps Sheet (all steps list)
    private var stepsSheet: some View {
        NavigationStack {
            List {
                if let route = viewModel.route {
                        ...
                        ForEach(Array(route.steps.enumerated()), id: \.offset) { index, step in
                            if !step.instructions.isEmpty {
                                HStack(alignment: .top, spacing: 15) {
                                    ZStack {
                                        Circle()
                                            .fill(index == viewModel.currentStepIndex && viewModel.isNavigating
                                                  ? Color.brown : Color.gray.opacity(0.3))
                                            .frame(width: 28, height: 28)
                                        Text("\(index + 1)")
                                            .font(.caption).fontWeight(.bold)
                                            .foregroundColor(index == viewModel.currentStepIndex && viewModel.isNavigating
                                                             ? .white : .primary)
                                    }
                                    VStack(alignment: .leading, spacing: 4) {
                                        Text(step.instructions).font(.subheadline)
                                    ...
                                    Spacer()
                                    if index == viewModel.currentStepIndex && viewModel.isNavigating {
    ...
    /// Picks a sensible SF Symbol based on the instruction text
    private func directionIcon(for instruction: String) -> String {
        let lowered = instruction.lowercased()
        if lowered.contains("turn left")        { return "arrow.turn.up.left" }
        if lowered.contains("turn right")       { return "arrow.turn.up.right" }
        if lowered.contains("slight left")      { return "arrow.up.left" }
        if lowered.contains("slight right")     { return "arrow.up.right" }
        if lowered.contains("u-turn")           { return "arrow.uturn.left" }
        if lowered.contains("merge")            { return "arrow.merge" }
        if lowered.contains("exit")             { return "arrow.turn.down.right" }
        if lowered.contains("roundabout")       { return "arrow.clockwise" }
        if lowered.contains("arrive") || lowered.contains("destination") { return "mappin.circle.fill" }
        if lowered.contains("head") || lowered.contains("continue") { return "arrow.up" }
        return "arrow.up"
    }
}
